...
class TestCredentials_new(TestCase):
    ...
    def test_scopes(self, from_client_config_mock: mock.Mock) -> None:
        flow_mock = from_client_config_mock.return_value
        assert isinstance(flow_mock, mock.Mock)
        scopes_list: list[list[str] | None] = [
            None,
            [],
        ...
        for scopes in scopes_list:
            ...
            with self.subTest(scopes=scopes):
                creds = credentials.Credentials.new(scopes=scopes)
                self.assertEqual(
                    creds._credentials,
                    flow_mock.run_local_server.return_value
                )
            from_client_config_mock.assert_called_once_with(
                credentials.CLIENT_CONFIG,
                scopes or credentials.SCOPES
            )
        ...
        for run_local_server in (True, False):
            with self.subTest(run_local_server=run_local_server):
                ...
                if run_local_server:
                    ...
                    self.assertEqual(
                        creds._credentials,
                        flow_mock.run_console.return_value
                    )
